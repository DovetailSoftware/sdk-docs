### BillingPeriodStartDate Property (UpdateScheduleSetup)

The billing period starting DateTime for the Schedule. If not set no change is made.

#### Syntax

```vbnet
'Declaration

<PropertyBitValueAttribute()>
Public Property BillingPeriodStartDate As Date
```

```csharp
[PropertyBitValueAttribute()]
public DateTime BillingPeriodStartDate {get; set;}
```

[!include[Requirements](../partials/requirements.md)]

#### Reference

[UpdateScheduleSetup Class](FChoice.Toolkits.Clarify~FChoice.Toolkits.Clarify.Contracts.UpdateScheduleSetup.md)  
[UpdateScheduleSetup Members](FChoice.Toolkits.Clarify~FChoice.Toolkits.Clarify.Contracts.UpdateScheduleSetup_members.md)