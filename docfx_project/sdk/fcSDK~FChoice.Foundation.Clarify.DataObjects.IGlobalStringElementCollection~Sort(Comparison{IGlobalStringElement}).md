### Sort(Comparison<IGlobalStringElement>) Method

Syntax

```vbnet
'Declaration

Overloads Sub Sort( _
   ByVal _comparison_ As Comparison(Of IGlobalStringElement) _
)
```

```csharp
void Sort( 
   Comparison<IGlobalStringElement> _comparison_
)
```

#### Parameters

_comparison_

[!include[Requirements](../partials/requirements.md)]

#### Reference

[IGlobalStringElementCollection Interface](fcSDK~FChoice.Foundation.Clarify.DataObjects.IGlobalStringElementCollection.md)  
[IGlobalStringElementCollection Members](fcSDK~FChoice.Foundation.Clarify.DataObjects.IGlobalStringElementCollection_members.md)  
[Overload List](fcSDK~FChoice.Foundation.Clarify.DataObjects.IGlobalStringElementCollection~Sort.md)